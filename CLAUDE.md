...
- **layered_png.dart**: `LayeredPng` — loads a Layered PNG (`.lpng`) asset (a ZIP of per-layer PNGs + `manifest.json` + thumbnail; see `scripts/lpng/`). Decodes every layer up front, exposes the layer tree (`LpngLayer`, groups vs images), `walk()` for depth-first traversal, `layerById()` for lookup (incl. nested), and rasterises the whole document or any single layer to a `ui.Image` via `toImage()` (honouring offsets, opacity, visibility, and `LpngBlendMode`). See `docs/layered_png.md`.
- **halftone.dart**: `HalftoneRenderer` — fragment-shader-driven halftone dot rendering (`HalftoneGradient`, `HalftoneStop`, `HalftoneGrowth`, `HalftoneBake`). Three uses: gradient fill, shape fill (`bake(gradient, path, {clip})` — `clip: true` slices dots per-pixel at the boundary, `clip: false` keeps whole dots that spill past it with `feather` silhouette AA), and image reproduction (`halftoneImage`). Call `load()` once, then apply `shaderFor`/`shaderForPathDots` as a `Paint.shader` in your own `render`, or `bake` to a cached `ui.Image` for blit-while-scrolling. `HalftoneGradient.offset` is a 2D lattice phase for seam alignment across baked slices. Pure `dart:ui`; ships the `shaders/halftone*.frag` programs (loaded via `packages/sizzle/shaders/...`). Bridges to `StrokePath.toPath()` for filling stroke-defined shapes. See `docs/halftone.md`.
