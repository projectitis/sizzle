...
import '../../display/svg.dart';

/// A service for loading and caching parsed [Svg] assets.
///
/// Mirrors the queue/load/cache shape of [ImageService] and [FileService].
/// Each parsed [Svg] is keyed in the cache by its asset path.
class SvgService {
  final List<String> _queued = <String>[];
  final Map<String, Svg> _cache = <String, Svg>{};
  final String assetFolder;
  final AssetBundle assetBundle;

  /// Create a new SVG service with path to [assetFolder] for loading SVGs.
  SvgService(this.assetFolder, {AssetBundle? assetBundle})
  ...
  Future<Svg> load({required String path, bool cache = true}) async {
  ...
  Svg? operator [](String path) => _cache[path];

  /// Get a parsed SVG from the cache by [path].
  Svg? get(String path) => _cache[path];
